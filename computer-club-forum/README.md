...
本项目是「北京服装学院计算机知识学习社官方网站」仓库下的**子项目**，
仓库整体的介绍与贡献流程见上一级目录的 [README.md](../README.md) 与 [CONTRIBUTING.md](../CONTRIBUTING.md)。
...
在阿里云 ECS 上以**管理员 PowerShell** 执行。把仓库克隆到服务器上任意目录，
然后进入本子目录（示例路径 `C:\CSClub-Bift\computer-club-forum`）：

```powershell
cd C:\CSClub-Bift\computer-club-forum    # 换成你实际克隆/解压后的路径
...
内网 IP 会按本机网卡自动推断，想精确指定加 `-ServerAddress 10.0.0.12`。
...
.\deploy\setup-server.ps1 -MySqlHost 'rm-xxxxxx.mysql.rds.aliyuncs.com' `
  -MySqlUser forum_app -MySqlPassword 'RDS账号口令' -MySqlRootUser forum_app
```
...
本地开发想要开机自启，用本目录的 `install-autostart.ps1`（记得先把 `FORUM_MYSQL_*` 写进系统环境变量）。
...
| 报 `未安装 MySQL 驱动 mysql2` | `node_modules` 缺失 | `npm install mysql2 --registry=https://registry.npmmirror.com` |
...
> 说明：`src/storage/mysql.mjs` 目前只做过语法与流程校验（开发机没有 MySQL 环境），
> 第一次部署时建议按下面顺序确认，每一步都有明确输出：
>
> ```powershell
...
| `data/forum.json` | 历史数据（迁移后留档为 `forum.json.migrated`）；**仓库里只保留空论坛** |

---

## 12. 提交代码前请注意

仓库根目录的 `.gitignore` 已排除依赖、日志、数据库文件、备份、证书与服务器配置（含数据库口令），
但有两点必须每次人工确认：

1. **`data/forum.json` 必须是空论坛**（`users` 与 `posts` 都是 `[]`）。它一旦被真实写入，
   就会包含成员昵称和聊天内容——这才是最容易被误提交的隐私数据。提交前检查：

   ```powershell
   node -e "const d=require('./data/forum.json'); console.log('用户',d.users.length,'帖子',d.posts.length)"
   ```

   不为 0 就先清空：`node server.mjs --clear`（或手工把两个数组改回 `[]`）。

2. **不要提交任何口令**：`forum.env.cmd`、`forum.mysql.env.cmd`、
   `deploy/mysql/01-schema.sql` 里若已改成真实口令，请改回占位符再提交。

另外，`deploy/` 下的 PowerShell 脚本都保存为**带 BOM 的 UTF-8**：
在代码页 936 的 Windows 上，PowerShell 5.1 会把无 BOM 的 UTF-8 中文提示读成乱码并报语法错误，
所以改这些脚本时请保持 UTF-8 BOM（根目录 `.editorconfig` 里已声明 `charset = utf-8-bom`）。
